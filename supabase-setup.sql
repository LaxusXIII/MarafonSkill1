...
create index if not exists runners_created_at_idx on public.runners (created_at desc);
create index if not exists runners_telegram_user_id_idx on public.runners (telegram_user_id);
...
for insert
to anon
with check (user_id is null and telegram_user_id is null and source = 'android');
...
-- Add your Google account as an admin:
